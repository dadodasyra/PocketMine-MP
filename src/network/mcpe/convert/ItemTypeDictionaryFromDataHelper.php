...
final class ItemTypeDictionaryFromDataHelper{
	...
	public static function loadFromString(string $data) : ItemTypeDictionary{
		...
		foreach(Utils::promoteKeys($table) as $name => $entry){
			...
			$params[] = new ItemTypeEntry($name, $entry["runtime_id"], $entry["component_based"], $entry["version"], $nbt === null ? $emptyNBT : new CacheableNbt($nbtSerializer->read(base64_decode($nbt, true))->mustGetCompoundTag()));
		}
